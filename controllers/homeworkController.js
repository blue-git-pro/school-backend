...
class homeworkController {
  async getHomeworks(req, res) {
    ...
    let addFields3 = {
      "$addFields": {
        "homeworkGradeObjectId": {
          "$toObjectId": "$homework_grade_id"
        }
      }
    }
    ...
    let homeworks = await db.aggregateData(HomeworkModel, query);
    res.json({
      status: true,
      result: homeworks
    });
  }
  async getHomeworksByGrade(req, res) {
    let {
      school_id
    } = req.headers;
    let match = {
  ...
  async getClonedHomeworks(req, res) {
    let {
      school_id
    } = req.headers;
    let match = {
  ...
  async addHomework(req, res) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      await HomeworkModel.create(data);
    }
    //  data['user_name'] = user_name
    //  await HomeworkModel.create(data);
    saveActivity(user_id, user_name, "Homework", "New Homework has been created.", "Created");
    res.json({
      status: true
    });
  }
  async getHomework(req, res) {
    ...
    let homeworkObj = await HomeworkModel.findOne({
      _id: id
    });
    res.json({
      status: true,
  ...
  async editHomework(req, res) {
    ...
    saveActivity(user_id, user_name, "Homework", "A Homework has been edited.", "Updated");
    res.json({
      status: true
    });
  }
  async deleteHomework(req, res) {
    ...
    await HomeworkModel.deleteOne({
      _id: id
    });
    saveActivity(user_id, user_name, "Homework", "An Homework has been deleted.", "Deleted");
    res.json({
      status: true
    });
  }
  async cloneHomework(req, res) {
    let data = req.body;
    data['created_at'] = Date.now();
    data['created_by'] = {
      name: req.headers.user_name,
    ...
    for (var i = 0; i < length; i++) {
      ...
      data['description'] = oldData[i].title
